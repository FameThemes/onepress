<?php
/**
 * Section: About (declarative list, merged in customize-option-definitions.php).
 *
 * @package onepress
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

if ( ! class_exists( 'Onepress_Config', false ) || ! Onepress_Config::is_section_active( 'about' ) ) {
	return array();
}

$ctx = isset( $GLOBALS['onepress_customize_builder_context'] ) && is_array( $GLOBALS['onepress_customize_builder_context'] )
	? $GLOBALS['onepress_customize_builder_context']
	: array();
$option_pages = isset( $ctx['option_pages'] ) && is_array( $ctx['option_pages'] )
	? $ctx['option_pages']
	: array( 0 => esc_html__( 'Select page', 'onepress' ) );

$definitions = array(
	array(
		'type'            => 'panel',
		'id'              => 'onepress_about',
		'priority'        => 160,
		'title'           => esc_html__( 'Section: About', 'onepress' ),
		'description'     => '',
		'active_callback' => 'onepress_showon_frontpage',
	),
	array(
		'type'        => 'section',
		'id'          => 'onepress_about_settings',
		'priority'    => 3,
		'title'       => esc_html__( 'Section Settings', 'onepress' ),
		'description' => '',
		'panel'       => 'onepress_about',
	),
	array(
		'id'          => 'onepress_about_disable',
		'control'     => 'wp',
		'input_type'  => 'checkbox',
		'section'     => 'onepress_about_settings',
		'label'       => esc_html__( 'Hide this section?', 'onepress' ),
		'description' => esc_html__( 'Check this box to hide this section.', 'onepress' ),
		'default'     => '',
	),
	array(
		'id'                => 'onepress_about_id',
		'control'           => 'wp',
		'input_type'        => 'text',
		'section'           => 'onepress_about_settings',
		'label'             => esc_html__( 'Section ID:', 'onepress' ),
		'description'       => esc_html__( 'The section ID should be English character, lowercase and no space.', 'onepress' ),
		'default'           => esc_html__( 'about', 'onepress' ),
		'sanitize_callback' => 'onepress_sanitize_text',
	),
	array(
		'id'          => 'onepress_about_title',
		'control'     => 'wp',
		'input_type'  => 'text',
		'section'     => 'onepress_about_settings',
		'label'       => esc_html__( 'Section Title', 'onepress' ),
		'description' => '',
		'default'     => esc_html__( 'About Us', 'onepress' ),
	),
	array(
		'id'          => 'onepress_about_subtitle',
		'control'     => 'wp',
		'input_type'  => 'text',
		'section'     => 'onepress_about_settings',
		'label'       => esc_html__( 'Section Subtitle', 'onepress' ),
		'description' => '',
		'default'     => esc_html__( 'Section subtitle', 'onepress' ),
	),
	array(
		'id'          => 'onepress_about_desc',
		'control'     => 'editor',
		'section'     => 'onepress_about_settings',
		'label'       => esc_html__( 'Section Description', 'onepress' ),
		'description' => '',
		'default'     => '',
		'setting'     => array(
			'sanitize_callback' => 'onepress_sanitize_text',
			'default'           => '',
		),
	),
);

if ( class_exists( 'OnePress_Plus' ) ) {
	$definitions[] = array(
		'id'          => 'onepress_about_layout',
		'control'     => 'wp',
		'input_type'  => 'select',
		'section'     => 'onepress_about_settings',
		'label'       => esc_html__( 'Layout Settings', 'onepress' ),
		'description' => '',
		'default'     => 3,
		'choices'     => array(
			4 => esc_html__( '4 Columns', 'onepress' ),
			3 => esc_html__( '3 Columns', 'onepress' ),
			2 => esc_html__( '2 Columns', 'onepress' ),
			1 => esc_html__( '1 Column', 'onepress' ),
		),
	);
}

$definitions[] = array(
	'type'     => 'callback',
	'callback' => static function ( $wp_customize ) {
		if ( function_exists( 'onepress_add_upsell_for_section' ) ) {
			onepress_add_upsell_for_section( $wp_customize, 'onepress_about_settings' );
		}
	},
);

$definitions[] = array(
	'type'        => 'section',
	'id'          => 'onepress_about_content',
	'priority'    => 6,
	'title'       => esc_html__( 'Section Content', 'onepress' ),
	'description' => '',
	'panel'       => 'onepress_about',
);

$definitions[] = array(
	'id'            => 'onepress_about_boxes',
	'control'       => 'repeatable',
	'label'         => esc_html__( 'About content page', 'onepress' ),
	'description'   => '',
	'section'       => 'onepress_about_content',
	'live_title_id' => 'content_page',
	'title_format'  => '[live_title]',
	'max_item'      => 3,
	'limited_msg'   => wp_kses_post(
		__( 'Upgrade to <a target="_blank" href="https://www.famethemes.com/plugins/onepress-plus/?utm_source=theme_customizer&utm_medium=text_link&utm_campaign=onepress_customizer#get-started">OnePress Plus</a> to be able to add more items and unlock other premium features!', 'onepress' )
	),
	'fields'        => array(
		'content_page' => array(
			'title'   => esc_html__( 'Select a page', 'onepress' ),
			'type'    => 'select',
			'options' => $option_pages,
		),
		'hide_title'   => array(
			'title' => esc_html__( 'Hide item title', 'onepress' ),
			'type'  => 'checkbox',
		),
		'enable_link'  => array(
			'title' => esc_html__( 'Link to single page', 'onepress' ),
			'type'  => 'checkbox',
		),
	),
	'transport'     => 'refresh',
	'setting'       => array(
		'sanitize_callback' => 'onepress_sanitize_repeatable_data_field',
		'transport'         => 'refresh',
	),
);

$definitions[] = array(
	'id'          => 'onepress_about_content_source',
	'control'     => 'wp',
	'input_type'  => 'select',
	'section'     => 'onepress_about_content',
	'label'       => esc_html__( 'Item content source', 'onepress' ),
	'description' => '',
	'default'     => 'content',
	'choices'     => array(
		'content' => esc_html__( 'Full Page Content', 'onepress' ),
		'excerpt' => esc_html__( 'Page Excerpt', 'onepress' ),
	),
);

return $definitions;
