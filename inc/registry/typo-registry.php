...
if (! function_exists('onepress_styling_typography_target_elements_registry')) {
	...
	function onepress_styling_typography_target_elements_registry()
	{
		...
		$categories = array(
			...
			'front_sections'    => esc_html__('Fontpage sections', 'onepress'),
		...
		for ($i = 1; $i <= 6; $i++) {
			$extra = '';
			if ($i == 2) {
				$extra = ', .section-news .entry-title, .archive .entry-title, .blog .entry-title';
			}
			$elements[] = [
				'id'       => 'h' . $i,
				'selector' => '.site-main h' . $i . $extra,
				'name'     => esc_html__('Heading ' . $i, 'onepress'),
				'category' => 'content',
			];
		}
		...
		$elements[] = [
			'id'       => 'frp_hero_heading',
			'selector' => '#hero .hero-large-text, #hero .h1, #hero h2',
			'name'     => esc_html__('Hero Heading', 'onepress'),
			'category' => 'front_sections',
		];
		$elements[] = [
			'id'       => 'frp_hero_desc',
			'selector' => '#hero .hero__content p, #hero .hero__content div',
			'name'     => esc_html__('Hero Description', 'onepress'),
			'category' => 'front_sections',
		];
		$elements[] = [
			'id'       => 'section_h',
			'selector' => '.onepage-section .section-title',
			'name'     => esc_html__('Section Heading', 'onepress'),
			'category' => 'front_sections',
		];
		$elements[] = [
			'id'       => 'section_subtitle',
			'selector' => '.onepage-section .section-subtitle',
			'name'     => esc_html__('Section Subtitle', 'onepress'),
			'category' => 'front_sections',
		];
		$elements[] = [
			'id'       => 'section_desc',
			'selector' => '.onepage-section .section-desc',
			'name'     => esc_html__('Section Description', 'onepress'),
			'category' => 'front_sections',
		];
		...
		$elements[] = [
			'id'       => 'footer_widget_h',
			'selector' => '#footer-widgets .widget-title, #footer-widgets .wp-block-heading',
			'name'     => esc_html__('Footer Heading', 'onepress'),
			'category' => 'footer',
		];
		...
		$elements[] = [
			'id'       => 'custom_text',
			'selector' => '.custom__content .custom-text',
			'name'     => esc_html__('Custom Text', 'onepress'),
			'category' => 'custom',
			'locked' => true,
			'multiple' => true,
