	...
	<section  id="<?php esc_attr( $id ); ?>" 
		...
		switch ( $layout ) {
			...
			default:
				...
				$target_1     = ( 1 == $btn_1_target ) ? 'target="_blank"' : '';
				$target_2     = ( 1 == $btn_2_target ) ? 'target="_blank"' : '';
				...
				<div class="container"<?php echo $hero_content_style; ?>>
					<div class="hero__content hero-content-style<?php echo esc_attr( $layout ); ?>">
						...
						<?php
						if ( '' != $hcl1_smalltext ) {
							echo '<p class="hero-small-text"> ' . do_shortcode( wp_kses_post( $hcl1_smalltext ) ) . '</p>';
						}
						?>
						<?php
						if ( '' != $hcl1_btn1_text && '' != $hcl1_btn1_link ) {
							echo '<a ' . $target_1 . ' href="' . esc_url( $hcl1_btn1_link ) . '" class="btn ' . esc_attr( $btn_1_style ) . ' btn-lg">' . wp_kses_post( $hcl1_btn1_text ) . '</a>';
						}
						?>
						<?php
						if ( '' != $hcl1_btn2_text && '' != $hcl1_btn2_link ) {
