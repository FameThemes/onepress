...
if ( ! $disable && ! empty( $data ) ) {
	...
	<?php if ( ! onepress_is_selective_refresh() ) { ?>
		<section id="<?php echo esc_attr( $id ); ?>" <?php do_action( 'onepress_section_atts', 'features' ); ?>
			...
			foreach ( $data as $k => $f ) {
				...
				<div class="feature-item col-lg-<?php echo esc_attr( $layout ); ?> col-sm-6 wow slideInUp">
					<div class="feature-media">
						...
						if ( $f['link'] ) {
							?>
							<a href="<?php echo esc_url( $f['link'] ); ?>"><?php } ?>
						<?php echo $media; ?>
						<?php
						if ( $f['link'] ) {
							?>
					...
					if ( $f['link'] ) {
						?>
						<a href="<?php echo esc_url( $f['link'] ); ?>"><?php } ?><?php echo esc_html( $f['title'] ); ?>
						<?php
						if ( $f['link'] ) {
							?>
						</a><?php } ?></h4>
