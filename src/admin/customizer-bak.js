	...
	$.fn.alphaColorPicker = function () {
		return this.each( function () {
			...
			$container
				...
				.on( 'click', function () {
					const alpha =
						acp_get_alpha_value_from_color( defaultColor );

					acp_update_alpha_value_on_alpha_slider(
						alpha,
						$alphaSlider
					);
				} );
		...
		const _editor = {
			...
			ready( container ) {
				...
				try {
					control.preview.html(
						window.switchEditors._wp_Autop( content )
					);
				} catch ( e ) {}
				...
				$( 'body' ).on(
					...
					function ( e ) {
						if ( ! $( e.target ).is( control.preview ) ) {
							...
							$( '.wp-js-editor-preview' ).removeClass(
								'wpe-focus'
							);
						}
					}
						...
						.wp_js_editor( {
							...
							init_instance_callback( editor ) {
								...
								w.on(
									...
									function ( e ) {
										e.preventDefault();
										control.editing_editor.removeClass(
											'wpe-active'
										);
										$(
											'.wp-js-editor-preview'
										).removeClass( 'wpe-focus' );
									}
								);
			...
			_init() {
				...
				control.editing_area.on( 'change', function () {
					control.preview.html(
						window.switchEditors._wp_Autop( $( this ).val() )
					);
				} );
	...
	function _remove_editor( $context ) {
		$( 'textarea', $context ).each( function () {
			...
			try {
				...
				if (
					typeof tinyMCEPreInit.mceInit[ editor_id ] !== 'undefined'
				) {
					delete tinyMCEPreInit.mceInit[ editor_id ];
				}

				if (
					typeof tinyMCEPreInit.qtInit[ editor_id ] !== 'undefined'
				) {
		...
		if ( _wpCustomizeSettings.autofocus ) {
			if ( _wpCustomizeSettings.autofocus.section ) {
				const id =
					...
					_wpCustomizeSettings.autofocus.section;
				_is_init_editors[ id ] = true;
				const section = $( '#' + id );
				setTimeout( function () {
					if ( $( '.wp-js-editor', section ).length > 0 ) {
...
jQuery( window ).ready( function ( $ ) {
	if ( typeof onepress_customizer_settings !== 'undefined' ) {
		if ( onepress_customizer_settings.number_action > 0 ) {
			$( '.control-section-themes h3.accordion-section-title' ).append(
				'<a class="theme-action-count" href="' +
					onepress_customizer_settings.action_url +
					'">' +
	 ...
	 */
	$( 'select[data-customize-setting-link="onepress_hero_layout"]' ).on(
		'change on_custom_load',
		function () {
			const v = $( this ).val() || '';
	...
	$( 'select[data-customize-setting-link="onepress_hero_layout"]' ).trigger(
		'on_custom_load'
	);
	 ...
	 */
	$( 'select[data-customize-setting-link="onepress_gallery_source"]' ).on(
		'change on_custom_load',
		function () {
			const v = $( this ).val() || '';
			...
			$(
				"li[id^='customize-control-onepress_gallery_settings_']"
			).hide();
			$(
				"li[id^='customize-control-onepress_gallery_source_" + v + "']"
			).show();
			$(
				"li[id^='customize-control-onepress_gallery_api_" + v + "']"
			).show();
			$(
				"li[id^='customize-control-onepress_gallery_settings_" +
					v +
	...
	$(
		'select[data-customize-setting-link="onepress_gallery_source"]'
	).trigger( 'on_custom_load' );

	/**
	 * For Gallery display settings
	 */
	$( 'select[data-customize-setting-link="onepress_gallery_display"]' ).on(
		'change on_custom_load',
		function () {
			const v = $( this ).val() || '';
			switch ( v ) {
				case 'slider':
					$(
						'#customize-control-onepress_g_row_height, #customize-control-onepress_g_col, #customize-control-onepress_g_spacing'
					).hide();
					break;
				case 'justified':
					$(
						'#customize-control-onepress_g_col, #customize-control-onepress_g_spacing'
					).hide();
					$( '#customize-control-onepress_g_row_height' ).show();
					break;
				case 'carousel':
					$(
						'#customize-control-onepress_g_row_height, #customize-control-onepress_g_col'
					).hide();
					$(
						'#customize-control-onepress_g_col, #customize-control-onepress_g_spacing'
					).show();
	...
	$(
		'select[data-customize-setting-link="onepress_gallery_display"]'
	).trigger( 'on_custom_load' );
} );
