...
( function ( factory ) {
	...
	JustifiedGallery.prototype.getSuffix = function ( width, height ) {
		...
		for ( i = 0; i < this.suffixRanges.length; i++ ) {
			if ( longestSide <= this.suffixRanges[ i ] ) {
				return this.settings.sizeRangeSuffixes[
					this.suffixRanges[ i ]
				];
			}
		}
	...
	JustifiedGallery.prototype.getUsedSuffix = function ( str ) {
		for ( const si in this.settings.sizeRangeSuffixes ) {
			if ( this.settings.sizeRangeSuffixes.hasOwnProperty( si ) ) {
				...
				if (
					this.endsWith( str, this.settings.sizeRangeSuffixes[ si ] )
				) {
	...
	JustifiedGallery.prototype.onEntryMouseEnterForCaption = function (
		...
		const $caption = this.captionFromEntry(
	...
	JustifiedGallery.prototype.analyzeImages = function ( isForResize ) {
		for (
			let i = this.lastAnalyzedIndex + 1;
