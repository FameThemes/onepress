...
( function ( root, factory ) {
	...
	( function () {
		...
		const defaults = {
			...
			/**
			 * @description number of preload slides
		...
		function Plugin( element, options ) {
			...
			} else if ( this.s.selector === 'this' ) {
				this.$items = this.$el;
			} else if ( this.s.selector !== '' ) {
				if ( this.s.selectWithin ) {
					this.$items = $( this.s.selectWithin ).find(
						this.s.selector
					);
				} else {
		...
		Plugin.prototype.build = function ( index ) {
			...
			$.each( $.fn.lightGallery.modules, function ( key ) {
				_this.modules[ key ] = new $.fn.lightGallery.modules[ key ](
					_this.el
				);
			} );
		...
		Plugin.prototype.structure = function () {
			...
			if ( this.doCss() ) {
				const $inner = this.$outer.find( '.lg-inner' );
		...
		Plugin.prototype.setTop = function () {
			if ( this.s.height !== '100%' ) {
				...
				const top = ( wH - parseInt( this.s.height, 10 ) ) / 2;
				const $lGallery = this.$outer.find( '.lg' );
		...
		/**
		 *  @description Check the given src is video
		 * @param          index
		 * @param {string} src
		...
		/**
		 *  @description Create image counter
		 *  Ex: 1/10
		 */
		...
		/**
		 *  @description add sub-html into the slide
		...
		/**
		 *  @description Load slide content into slide.
		 * @param {number}  index - index of the slide.
		...
		Plugin.prototype.loadContent = function ( index, rec, delay ) {
			...
			const _isVideo = _this.isVideo( _src, index );
			...
			_this.$slide
				...
				.on( 'load.lg error.lg', function () {
					...
					setTimeout( function () {
						...
						_this.$el.trigger( 'onSlideItemLoad.lg', [
							index,
							delay || 0,
						] );
					}, _speed );
		...
		/**
		 *   @description slide function for lightgallery
		...
		Plugin.prototype.slide = function (
			...
			const _prevIndex = this.$outer.find( '.lg-current' ).index();
			const _this = this;
			...
			const _length = this.$slide.length;
			const _time = _this.lGalleryOn ? this.s.speed : 0;
		...
		Plugin.prototype.goToNextSlide = function ( fromTouch ) {
			...
			if ( ! _this.lgBusy ) {
				if ( _this.index + 1 < _this.$slide.length ) {
					_this.index++;
					_this.$el.trigger( 'onBeforeNextSlide.lg', [
						_this.index,
					] );
		...
		Plugin.prototype.goToPrevSlide = function ( fromTouch ) {
			...
			if ( ! _this.lgBusy ) {
				if ( _this.index > 0 ) {
					_this.index--;
					_this.$el.trigger( 'onBeforePrevSlide.lg', [
						_this.index,
						fromTouch,
					] );
...
( function ( $, window, document, undefined ) {
	...
	Video.prototype.init = function () {
		...
		_this.core.$el.on(
			'hasVideo.lg.tm',
			function ( event, index, src, html ) {
				_this.core.$slide
					.eq( index )
					.find( '.lg-video' )
					.append(
						_this.loadVideo( src, 'lg-object', true, index, html )
					);
				if ( html ) {
		...
		const loadOnClick = function ( $el ) {
			...
			if (
				...
				if ( ! $el.hasClass( 'lg-has-video' ) ) {
					...
					const $tempImg = $el.find( '.lg-object' );
					...
					const youtubePlayer = $el.find( '.lg-youtube' ).get( 0 );
					const vimeoPlayer = $el.find( '.lg-vimeo' ).get( 0 );
					const dailymotionPlayer = $el
						.find( '.lg-dailymotion' )
						.get( 0 );
					const html5Player = $el.find( '.lg-html5' ).get( 0 );
					if ( youtubePlayer ) {
						...
						dailymotionPlayer.contentWindow.postMessage(
							'play',
							'*'
						);
					} else if ( html5Player ) {
		...
		if (
			...
			_this.core.$el.on( 'onSlideClick.lg.tm', function () {
				const $el = _this.core.$slide.eq( _this.core.index );
				loadOnClick( $el );
			} );
		} else {
			// For IE 9 and bellow
		...
		_this.core.$el.on(
			...
			function ( event, prevIndex, index ) {
				const $videoSlide = _this.core.$slide.eq( prevIndex );
				const youtubePlayer = $videoSlide
					.find( '.lg-youtube' )
					.get( 0 );
				const vimeoPlayer = $videoSlide.find( '.lg-vimeo' ).get( 0 );
				const dailymotionPlayer = $videoSlide
					.find( '.lg-dailymotion' )
					.get( 0 );
				const vkPlayer = $videoSlide.find( '.lg-vk' ).get( 0 );
				const html5Player = $videoSlide.find( '.lg-html5' ).get( 0 );
				if ( youtubePlayer ) {
					youtubePlayer.contentWindow.postMessage(
				...
				const _isVideo = _this.core.isVideo( _src, index ) || {};
				if (
					_isVideo.youtube ||
					_isVideo.vimeo ||
